...
def make_pipeline():
    ...
    combined_factor = (
        (0.1*value_winsorized + 
        0.1*quality_winsorized + 
        0.2*sentiment_score_winsorized + 
        0.3*growth_score_winsorized +
        0.1*dps_growth_winsorized +
        0.2*asst_turnover_winsorized
        )
    )
...
def rebalance(context, data):
    ...
    net_exposure = opt.NetExposure(-0.08, 0.07)
    
    constraints.append(net_exposure)
